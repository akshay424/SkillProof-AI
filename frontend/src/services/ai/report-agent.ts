...
export async function synthesizeTaskReport(
  ...
  return completeJSON<TaskReportSynthesis>("final_evaluation", user, TASK_SYNTHESIS_PROMPT);
}
...
export async function synthesizePeriodReport(
  ...
  return completeJSON<{ summary: string; overallScore: number }>("weekly_evaluation", user, PERIOD_SYNTHESIS_PROMPT);
}
