...
import { readResume, type ResumeReaderResult } from "@/services/ai/resume-reader-agent";
...
export interface GeneratedRoadmap {
  title: string;
  weeks: GeneratedWeek[];
  resumeAnalysis?: ResumeReaderResult;
}

const SYSTEM_PROMPT = `You are the SkillFlow AI Roadmap Creator — First Day Agent.
Create a first-day diagnostic roadmap for a fresher AI Product Developer.
Resume skills are unverified claims. Use the Resume Reader result and interview evaluation to choose
up to three foundation competencies to validate first. Do not label the fresher weak. Tasks must be
small, employee-facing, practical, and suitable for a fresher. Include context, requirements,
acceptance criteria, and resources. Respond with strict JSON matching this exact shape, no prose outside JSON:
{
  "title": string,
  "weeks": [
    {
...
export async function generateRoadmap(resumeText: string, interviewNotes: string): Promise<GeneratedRoadmap> {
  ...
  const resume = await readResume(resumeText);
  const user = JSON.stringify({
    employeeType: "fresher",
    targetRole: "AI Product Developer",
    resumeReaderResult: resume,
    interviewEvaluation: interviewNotes,
