...
import { useCreateEvaluationReport } from "@/services/queries/reports";
  ...
  const finishUp = async (items: VivaQuestion[]) => {
    ...
    try {
      ...
      await createReport.mutateAsync({
        ...
        folder_structure: evaluation.folderStructure,
        problem_solving: evaluation.problemSolving,
        code_quality: evaluation.codeQuality,
        ai_usage: evaluation.aiUsage,
        evidence: evaluation.evidence,
        suggestions: synthesis.finalSuggestions,
        summary: synthesis.communicationVerdict,
      ...
      await recordScores.mutateAsync({
        userId,
        scores: [{ skillName: weekTheme, score: synthesis.overallScore }],
        source: "task_evaluation",
      });

      await completeTask.mutateAsync({ userId, taskId: task.id });
        ...
        >
          ...
          {stage === "done" && (
            <div className="flex flex-col items-center gap-3 py-8 text-center">
              ...
              <p className="text-sm text-muted-foreground">
                Your report has been added to Reports and this task is marked complete.
              </p>
